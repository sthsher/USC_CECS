...
heap<T> :: heap (int d, bool maxHeap)
{
	factor = d;
	isMax = maxHeap;
	arrayPtr = new doubleSizeArray<T>;
	size = 0;
}
...
template <class T>
int heap<T> :: largestChildIndex(T* childArray, int arraySize, int firstIndex, bool(*compareFunction)(T, T))
{
	...
	{
		if (compareFunction(childArray[i], tempLargest))
...
template <class T>
int heap<T> :: smallestChildIndex(T* childArray, int arraySize, int firstIndex, bool(*compareFunction)(T, T))
{
	//childArray points to an array containing all the values to be compared
	...
	for (int i = 1; i < arraySize; ++i)
	{
		if (compareFunction(tempLargest, childArray[i]))
		{
			//set as new smallest
			tempLargest = childArray[i];
			//update index
...
//this is for delete
template <class T>
void heap<T> :: trickleDown(int currentNode, bool(*compareFunction)(T, T))
{
...
*/
	...
	if (isMax)
	{
		childIndex = largestChildIndex(childArray, arraySize, firstChild, compareFunction);

		T childItem = arrayPtr->get(childIndex);

		//maxHeap base case: if the largest child is still smaller than the current
		if (compareFunction(currentItem, childItem))
		{
			delete [] childArray;
			return;
		}
	...
	{
		childIndex = smallestChildIndex(childArray, arraySize, firstChild, compareFunction);
		
		T childItem = arrayPtr->get(childIndex);

		//minHeap base case: if the smallest child is still larger than the current
		if ( compareFunction(childItem, currentItem))
		{
			delete [] childArray;
			return;
		}	
	...
	delete [] childArray;
	return trickleDown(childIndex, compareFunction);
...
template <class T>
void heap<T> :: trickleUp(int currentNode, bool(*compareFunction)(T, T))
{
	...
	if (isMax)
	{
		...
		if (compareFunction(parentItem, currentItem))
		{
			return;
		}
		else
		{
			...
			return trickleUp(parentIndex, compareFunction);
		}
	}
	else
	{
		//if the parent is smaller, base case: no need to  swap
		if (compareFunction(currentItem, parentItem))
		{
			return;
		}
		else
...
template <class T>
void heap<T> ::  remove (bool(*compareFunction)(T, T))
{
...
template <class T>
void heap<T> ::  add (const T &item, bool(*compareFunction)(T, T))
{
	...
	trickleUp(size++, compareFunction);

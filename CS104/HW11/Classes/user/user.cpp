...
void user::printUserInfo()
{
	...
	cout << "    Date of Birth: " << userInfo[2] << endl;
	cout << "--------------------------------------------------" << endl;
...
bool user::operator==(const user &other)
{
	...
	if (this->username == other.returnUsername())
		return true;
	else
	{
		return false;
...
int user::hash() const
{
	//when getting users, only field will be fullname
	//therefore only hash fullname
	...
	{
		value = abs(value*(int)username[i]);
	}

	return value;
}

void user::setSecurityQ(string s)
{
	securityQ = s;
}
void user::setSecurityA(string s)
{
	securityA = s;
}

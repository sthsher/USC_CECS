...
{
	// notice: I changed the partition function a bit from class.
	// The one from class assumed that the pivot was in position r.
	// Since we're assuming that anyway, I refrained from passing the pivot,
	// and instead just choose p = a[r] inside the function.
	int i = left;
	T p = array[right];
...
void quickSort<T>::qSort (T array[], int left, int right)
{
	if (left < right)
	{
		...
		//printArray(array, left, right, mid);

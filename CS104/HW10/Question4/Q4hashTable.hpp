...
template <class keyT, class valueT>
void hashTable<keyT, valueT>::printHighestFrequency(int size)
{
	...
	for (int i = 0; i < tableSize; ++i)
	{
		...
		{
			//create a checker
			node<keyT, valueT>* checkNode = this->at(i);
			while (checkNode != NULL)
			{
				if (checkNode->value == highestFrequency)
					array[num++] = checkNode->key.content;

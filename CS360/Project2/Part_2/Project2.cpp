...
Project2::Project2(Simulator* sim1) {
	...
	std::vector<Point2D> obstacleVec = sim->getKnownObstacleLocations();
...
RobotAction Project2::getOptimalAction(Simulator* sim1, Robot* r1) {
	...
	int robotX = (int)sim->getRobot()->X;
	int robotY = (int)sim->getRobot()->Y;
	...
	AStarNode* node = AREA[(int)sim->getTarget().x][(int)sim->getTarget().y];
	while (node != NULL){
		node->print = true;
		node = node->GetParent();
	}

	AREA[(int)sim->getRobot()->X][(int)sim->getRobot()->Y]->print = true;
	
	for (int i = 0; i < sim->SY; ++i){
		if (i / 10 == 0){
			std::cout << " " << i << " ";
		}else{
			std:: cout << " " << i;
...
void Project2::AStar(Point2D point){
...
void Project2::GenerateNeighbors(){
	for (int i = 0; i < sim->SX; ++i){
		for (int j = 0; j < sim->SY; ++j){
...
void Project2::GenerateHeuristics(){

	//Going through walls
	float goalX = (float)sim->getTarget().x;
	float goalY = (float)sim->getTarget().y;

...
void equateOffset(GzOffset LHS, const GzOffset RHS)
{
	LHS[0] = RHS[0];
	LHS[1] = RHS[1];
	LHS[2] = RHS[2];
...
int Application5::Initialize()
{
	...
	GzToken		nameListOffset[3];
	GzPointer	valueListOffset[3];
	...
	for (int i = 0; i < AAKERNEL_SIZE; ++i)
	{
		...
		*/
		...
		/* Setup Offsets */
		float offsetX = AAFilter[i][0];
		float offsetY = AAFilter[i][1];
		float weight = AAFilter[i][2];
		nameListOffset[0] = GZ_AASHIFTX;
		valueListOffset[0] = (GzPointer)&offsetX;
		nameListOffset[1] = GZ_AASHIFTY;
		valueListOffset[1] = (GzPointer)&offsetY;
		nameListOffset[2] = GZ_AAWEIGHT;
		valueListOffset[2] = (GzPointer)&weight;

		status |= GzPutAttribute(m_pRender[i], 3, nameListOffset, valueListOffset);
	}
...
int Application5::Render()
{
	...
	GzDisplay* masterDisplay;
	GzNewDisplay(&masterDisplay, m_nWidth, m_nHeight);
	GzInitDisplay(masterDisplay);
	...
	for (int i = 0; i < m_nWidth * m_nHeight; ++i)
	{
		...
		{
			red += m_pRender[j]->display->fbuf[i].red * m_pRender[j]->weight;
			green += m_pRender[j]->display->fbuf[i].green * m_pRender[j]->weight;
			blue += m_pRender[j]->display->fbuf[i].blue * m_pRender[j]->weight;
			alpha += m_pRender[j]->display->fbuf[i].alpha * m_pRender[j]->weight;
			z += m_pRender[j]->display->fbuf[i].z * m_pRender[j]->weight;
		}
		masterDisplay->fbuf[i].red = red;
		masterDisplay->fbuf[i].green = green;
		masterDisplay->fbuf[i].blue = blue;
		masterDisplay->fbuf[i].alpha = alpha;
		masterDisplay->fbuf[i].z = z;
	}
	GzFlushDisplay2File(outfile, masterDisplay); 	/* write out or update display to file*/
	GzFlushDisplay2FrameBuffer(m_pFrameBuffer, masterDisplay);	// write out or update display to frame buffer
